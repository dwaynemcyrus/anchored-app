...
22. [ ] **Epic: Recover from concurrent external edits**
    - Outcome: Editing the same Markdown file in Anchored and Obsidian gives
      an early, understandable, and recoverable choice instead of waiting for
      an autosave failure. Anchored never silently overwrites the external
      version, and unresolved conflicts stop repeated autosave attempts.
    - Product decision: The first implementation creates one unique recovery
      copy of Anchored's local draft for each unresolved conflict, preserves
      the external file in place, and exposes explicit actions to open the
      recovery copy or reload the external version. Automatic merging is a
    ...
    22B. [x] **Detect external changes and serialize Anchored saves**
        ...
        - Verify: Native tests cover atomic replacement, Obsidian-style writes,
          ...
          vault for all byte-level checks.
        - Risk/rollback: Filesystem notifications can be duplicated, delayed,
          or generated by Anchored's own atomic rename. Treat events as hints;
    ...
    22C. [x] **Add frontend conflict state and recovery actions**
        ...
        - Change: Mark a loaded note as externally changed as soon as it is
          observed. Reload automatically only when there are no local edits.
          When both versions changed, preserve the editor draft, stop
          autosave, create at most one recovery copy, and show actions for
          opening the copy, reloading the external file, or explicitly using
          Save As. Keep conflict history actionable until resolution.
        - Verify: App tests cover clean external reloads, dirty external
          conflicts, repeated events, focus refresh, document switching,
          closing/reopening, keyboard access, error states, and no data loss
          when the recovery-copy command fails.
        - Risk/rollback: Reloading a dirty editor can lose local work, while
          leaving it dirty can block normal navigation. Require explicit
    ...
    22D. [ ] **Prototype conservative three-way merge**
        ...
        - Verify: Unit and fixture tests cover insertions, deletions, adjacent
          ...
          Confirm merged writes remain atomic and expected-content protected.
        - Risk/rollback: A faulty merge can silently combine the wrong text.
          Ship recovery-copy handling first, gate merge behind tests and a
          clear fallback, and disable automatic merge for any unproven case.

    22E. [ ] **Native QA, documentation, and release gate**
        - Expected files: `docs/TEST_CHECKLIST.md`, `docs/FEATURES.md`,
          `CHANGELOG.md`, `PLANS.md`, and the implementation/test files above.
        - Change: Add a two-editor Obsidian/Anchored smoke workflow, document
          the conflict actions and recovery location, and record whether the
          initial release includes merge or only conflict copies.
        - Verify: Format, lint, type-check, frontend tests, Rust tests,
          Clippy, production builds, disposable-vault byte-diff checks,
          keyboard/accessibility checks, and manual two-editor tests at the
          supported viewport and baseline Mac. Test normal external edits,
          Obsidian rewrites, line-ending changes, repeated autosaves, and
          crashes during recovery-copy creation.
        - Expected changelog: Describe early external-change detection,
